...
psa_drv_se_t secure_elements[MAX_NUM_SECURE_ELEMENTS];
                                         ...
                                         psa_key_attributes_t *attributes )
{
    ...
    if( psa_get_se_driver( slot->lifetime, NULL, NULL ) )
    {
        /* We always allow applications to see which slot number
         * contains a key. */
                             ...
                             psa_key_handle_t *handle )
{
    ...
    if( driver != NULL )
    {
        ...
        status = drv->key_management->p_import(
            ...
            data, data_length, &slot->data.se.bits );
        if( status != PSA_SUCCESS )
            goto exit;
                               ...
                               psa_key_handle_t *handle )
{
    ...
    status = psa_start_key_creation( PSA_KEY_CREATION_GENERATE, attributes, handle, &slot, &driver );
    if( status != PSA_SUCCESS )
        goto exit;
